...
UPDATE public.calls SET type = CASE WHEN COALESCE(max_participants, 2) <= 2 THEN 'direct' ELSE 'conference' END WHERE type IS NULL;
UPDATE public.calls SET media_backend = CASE WHEN COALESCE(max_participants, 2) <= 2 THEN 'p2p' ELSE 'livekit' END WHERE media_backend IS NULL;
UPDATE public.calls SET status = 'ringing' WHERE status = 'waiting';

ALTER TABLE public.calls ALTER COLUMN type SET DEFAULT 'direct', ALTER COLUMN media_backend SET DEFAULT 'p2p';
ALTER TABLE public.calls DROP CONSTRAINT IF EXISTS calls_type_check, DROP CONSTRAINT IF EXISTS calls_status_check, DROP CONSTRAINT IF EXISTS calls_media_backend_check, DROP CONSTRAINT IF EXISTS calls_backend_type_check;
ALTER TABLE public.calls
...
ALTER TABLE public.calls ADD CONSTRAINT calls_ended_by_fkey FOREIGN KEY (ended_by) REFERENCES public.profiles(id);
...
INSERT INTO public.call_participants (id, call_id, user_id, role, connection_status, is_muted, is_video_on, is_screen_sharing, joined_at, left_at, created_at)
SELECT p.id,p.call_id,p.user_id,CASE WHEN p.user_id=c.host_id THEN 'host' ELSE 'participant' END,CASE WHEN p.left_at IS NOT NULL THEN 'left' ELSE 'connected' END,p.is_muted,p.is_video_on,p.is_screen_sharing,p.joined_at,p.left_at,COALESCE(p.joined_at,c.created_at)
FROM public.call_participants_legacy p JOIN public.calls c ON c.id=p.call_id
...
CREATE TABLE public.call_invites (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(call_id,invitee_id), CHECK (inviter_id <> invitee_id)
);
...
FROM public.call_participants_legacy p JOIN public.calls c ON c.id=p.call_id WHERE p.user_id<>c.host_id
...
CREATE TABLE public.stream_subscribers (
  ...
  user_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  subscribed_at timestamptz NOT NULL DEFAULT now(), notified_at timestamptz,
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$ SELECT EXISTS(SELECT 1 FROM public.call_participants WHERE call_id=_call_id AND user_id=_user_id); $$;
CREATE OR REPLACE FUNCTION public.is_call_invitee(_call_id uuid,_user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$ SELECT EXISTS(SELECT 1 FROM public.call_invites WHERE call_id=_call_id AND invitee_id=_user_id); $$;
...
BEGIN
 ...
 INSERT INTO public.calls(id,type,host_id,conversation_id,status,media_backend,max_participants,metadata) VALUES(gen_random_uuid(),p_type,auth.uid(),p_conversation_id,'ringing',p_media_backend,p_max_participants,'{}') RETURNING * INTO v_call;
 INSERT INTO public.call_participants(call_id,user_id,role,connection_status) VALUES(v_call.id,auth.uid(),'host','disconnected');
 FOREACH uid IN ARRAY COALESCE(p_invitee_ids,'{}'::uuid[]) LOOP INSERT INTO public.call_invites(call_id,inviter_id,invitee_id) VALUES(v_call.id,auth.uid(),uid); END LOOP;
 RETURN v_call;
END; $$;

CREATE OR REPLACE FUNCTION public.rpc_invite_participants(p_call_id uuid,p_invitee_ids uuid[])
RETURNS SETOF public.call_invites LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE c public.calls; uid uuid; active_count integer;
BEGIN
 SELECT * INTO c FROM public.calls WHERE id=p_call_id FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'call_not_found'; END IF;
 ...
 SELECT count(*) INTO active_count FROM public.call_participants WHERE call_id=p_call_id AND connection_status IN ('connecting','connected');
 IF active_count+COALESCE(array_length(p_invitee_ids,1),0)>c.max_participants THEN RAISE EXCEPTION 'participant_limit_reached'; END IF;
 FOREACH uid IN ARRAY COALESCE(p_invitee_ids,'{}'::uuid[]) LOOP IF uid<>auth.uid() THEN INSERT INTO public.call_invites(call_id,inviter_id,invitee_id) VALUES(p_call_id,auth.uid(),uid) ON CONFLICT DO NOTHING; END IF; END LOOP;
...
RETURNS public.call_invites LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE i public.call_invites; out_i public.call_invites;
BEGIN
 IF p_response NOT IN ('accepted','declined') THEN RAISE EXCEPTION 'invalid_state_transition'; END IF;
 SELECT * INTO i FROM public.call_invites WHERE id=p_call_invite_id FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'invite_not_found'; END IF;
 ...
 IF p_response='accepted' THEN INSERT INTO public.call_participants(call_id,user_id,role,connection_status) VALUES(i.call_id,auth.uid(),'participant','connecting') ON CONFLICT(call_id,user_id) DO UPDATE SET connection_status='connecting',left_at=NULL;
 ELSE IF NOT EXISTS(SELECT 1 FROM public.call_participants WHERE call_id=i.call_id AND connection_status IN ('connecting','connected')) AND NOT EXISTS(SELECT 1 FROM public.call_invites WHERE call_id=i.call_id AND id<>i.id AND status IN ('pending','ringing','accepted')) THEN UPDATE public.calls SET status='declined',ended_at=COALESCE(ended_at,now()) WHERE id=i.call_id AND status='ringing'; END IF; END IF;
 RETURN out_i;
END; $$;
...
BEGIN
 SELECT * INTO c FROM public.calls WHERE id=p_call_id FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'call_not_found'; END IF;
 SELECT * INTO p FROM public.call_participants WHERE call_id=p_call_id AND user_id=auth.uid() FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'not_call_participant'; END IF;
 ...
 UPDATE public.calls SET status='active',started_at=COALESCE(started_at,now()) WHERE id=p_call_id RETURNING * INTO out_c; RETURN out_c;
END; $$;
...
BEGIN
 ...
 IF remaining=0 AND c.status NOT IN ('ended','declined','missed') THEN UPDATE public.calls SET status='ended',ended_at=COALESCE(ended_at,now()),ended_by=NULL WHERE id=p_call_id RETURNING * INTO out_c; ELSE SELECT * INTO out_c FROM public.calls WHERE id=p_call_id; END IF;
 RETURN out_c;
END; $$;
...
RETURNS public.call_participants LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE p public.call_participants;
BEGIN
 SELECT * INTO p FROM public.call_participants WHERE call_id=p_call_id AND user_id=auth.uid() FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'not_call_participant'; END IF;
 UPDATE public.call_participants SET is_muted=p_is_muted,is_video_on=p_is_video_on,is_screen_sharing=p_is_screen_sharing,is_hand_raised=p_is_hand_raised WHERE id=p.id RETURNING * INTO p; RETURN p;
END; $$;
...
DROP POLICY IF EXISTS "rtc_participants_media_update" ON public.call_participants;
DROP POLICY IF EXISTS "rtc_invites_update_own_response" ON public.call_invites;
...
REVOKE ALL ON public.call_participants FROM authenticated;
REVOKE ALL ON public.call_invites FROM authenticated;
GRANT SELECT ON public.calls,public.call_participants,public.call_invites TO authenticated;
GRANT UPDATE(is_muted,is_video_on,is_screen_sharing,is_hand_raised) ON public.call_participants TO authenticated;
