...
func (a *app) rtcCallRoute(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 {
		...
		case "join":
			if r.Method == http.MethodPost {
				a.rtcRPCCall(w, r, "rpc_mark_call_connected", callID)
				return
			}
		case "connected":
			if r.Method == http.MethodPost {
		...
		case "leave":
			if r.Method == http.MethodPost {
				a.rtcRPCCall(w, r, "rpc_leave_call", callID)
				return
			}
		case "end":
			if r.Method == http.MethodPost {
				a.rtcRPCCall(w, r, "rpc_end_call_for_everyone", callID)
				return
			}
		case "media":
			if r.Method == http.MethodPost {
				a.rtcMedia(w, r, callID)
				return
			}
		case "livekit-token":
			if r.Method == http.MethodGet || r.Method == http.MethodPost {
				a.rtcLiveKitToken(w, r, callID)
				return
			}
		case "participants":
			if r.Method == http.MethodGet {
				a.rtcGetParticipants(w, r, callID)
				return
			}
		case "invite":
			if r.Method == http.MethodPost {
				a.rtcInvite(w, r, callID)
				return
			}
		case "respond":
			if r.Method == http.MethodPost {
				a.rtcRespond(w, r, callID)
				return
			}
		}
	}
...
func (a *app) rtcCreateCall(w http.ResponseWriter, r *http.Request) {
	...
	if in.MaxParticipants == 0 {
		in.MaxParticipants = 2
	}
	if in.MediaBackend == "" {
		if in.Type == "conference" {
			in.MediaBackend = "livekit"
		} else {
			in.MediaBackend = "p2p"
		}
	}
	...
	if err := a.supabaseRPC(r.Context(), r, "rpc_create_call", args, &call); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	if in.Type == "conference" {
		callID, _ := call["id"].(string)
		room := "alsamos-call-" + callID
		if err := a.supabaseRPC(r.Context(), r, "rpc_set_call_room_name", map[string]any{"p_call_id": callID, "p_room_name": room}, &call); err != nil {
			writeSupabaseRTCError(w, err)
			return
		}
	}
...
func (a *app) rtcGetCall(w http.ResponseWriter, r *http.Request, callID string) {
	...
	if err := a.supabaseTable(r.Context(), r, "calls", "select=*\u0026id=eq."+callID+"\u0026limit=1", &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	if len(out) == 0 {
...
func (a *app) rtcGetParticipants(w http.ResponseWriter, r *http.Request, callID string) {
	var out []map[string]any
	if err := a.supabaseTable(r.Context(), r, "call_participants", "select=*\u0026call_id=eq."+callID+"\u0026order=created_at.asc", &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
...
func (a *app) rtcIncomingCalls(w http.ResponseWriter, r *http.Request) {
	...
	if err := a.supabaseTable(r.Context(), r, "call_invites", query, &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
...
func (a *app) rtcInvite(w http.ResponseWriter, r *http.Request, callID string) {
	...
	var out []map[string]any
	if err := a.supabaseRPC(r.Context(), r, "rpc_invite_participants", map[string]any{"p_call_id": callID, "p_invitee_ids": in.InviteeIDs}, &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"invites": out})
...
func (a *app) rtcRespond(w http.ResponseWriter, r *http.Request, callID string) {
	...
	if err := a.supabaseTable(r.Context(), r, "call_invites", query, &invites); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	if len(invites) == 0 {
		writeRTCError(w, http.StatusForbidden, "INVITE_NOT_FOR_USER", "invite not found for current user")
		return
	}
	inviteID, _ := invites[0]["id"].(string)
	var out map[string]any
	if err := a.supabaseRPC(r.Context(), r, "rpc_respond_to_invite", map[string]any{"p_call_invite_id": inviteID, "p_response": in.Response}, &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, out)
...
func (a *app) rtcMedia(w http.ResponseWriter, r *http.Request, callID string) {
	...
	var out map[string]any
	if err := a.supabaseRPC(r.Context(), r, "rpc_set_call_media_state", map[string]any{"p_call_id": callID, "p_is_muted": in.Muted, "p_is_video_on": in.VideoOn, "p_is_screen_sharing": in.ScreenSharing, "p_is_hand_raised": in.HandRaised}, &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, out)
...
func (a *app) rtcRPCCall(w http.ResponseWriter, r *http.Request, fn, callID string) {
	var out map[string]any
	if err := a.supabaseRPC(r.Context(), r, fn, map[string]any{"p_call_id": callID}, &out); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, out)
...
func (a *app) rtcLiveKitToken(w http.ResponseWriter, r *http.Request, callID string) {
	...
	if err := a.supabaseTable(r.Context(), r, "calls", query, &calls); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	if len(calls) == 0 {
	...
	c := calls[0]
	if c["type"] != "conference" || c["media_backend"] != "livekit" {
		writeRTCError(w, http.StatusBadRequest, "INVALID_MEDIA_BACKEND", "call is not a LiveKit conference")
		return
	}
	var room string
	...
	if err := a.supabaseTable(r.Context(), r, "call_participants", "select=id\u0026call_id=eq."+callID+"\u0026user_id=eq."+claimsFromContext(r.Context()).Sub+"\u0026limit=1", &participants); err != nil {
		writeSupabaseRTCError(w, err)
		return
	}
	if len(participants) == 0 {
		var invites []map[string]any
		if err := a.supabaseTable(r.Context(), r, "call_invites", "select=id\u0026call_id=eq."+callID+"\u0026invitee_id=eq."+claimsFromContext(r.Context()).Sub+"\u0026status=eq.accepted\u0026limit=1", &invites); err != nil {
			writeSupabaseRTCError(w, err)
			return
		}
		if len(invites) == 0 {
			writeRTCError(w, http.StatusForbidden, "FORBIDDEN", "user is not an eligible call participant")
			return
		}
	}
	publish := true
	...
	at.SetIdentity(claimsFromContext(r.Context()).Sub).SetValidFor(10 * time.Minute).SetVideoGrant(&auth.VideoGrant{RoomJoin: true, Room: room, CanPublish: &publish, CanSubscribe: &subscribe})
	token, err := at.ToJWT()
	if err != nil {
		writeRTCError(w, http.StatusInternalServerError, "TOKEN_ERROR", "failed to mint LiveKit token")
		return
	}
...
func (a *app) supabaseRPC(ctx context.Context, r *http.Request, fn string, args map[string]any, out any) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, a.supabaseURL+"/rest/v1/rpc/"+fn, bytes.NewReader(body))
	if err != nil {
		return err
	}
	a.applySupabaseAuth(req, r)
...
func (a *app) supabaseTable(ctx context.Context, r *http.Request, table, query string, out any) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, a.supabaseURL+"/rest/v1/"+table+"?"+query, nil)
	if err != nil {
		return err
	}
	a.applySupabaseAuth(req, r)
...
func (a *app) doSupabase(req *http.Request, out any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		var e supabaseRPCError
		_ = json.Unmarshal(data, &e)
		if e.Message == "" {
			e.Message = string(data)
		}
		return fmt.Errorf("supabase:%d:%s", resp.StatusCode, e.Message)
	}
	if out == nil {
		return nil
	}
...
func writeSupabaseRTCError(w http.ResponseWriter, err error) {
	...
	if strings.HasPrefix(msg, "supabase:") {
...
func writeRTCError(w http.ResponseWriter, status int, code, message string) {
	writeJSON(w, status, map[string]any{"error": rtcError{Code: code, Message: message}})
}
