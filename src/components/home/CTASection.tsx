      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            ...
            <Button asChild size="lg" className="h-14 px-8 text-lg bg-primary hover:bg-primary-dark text-primary-foreground rounded-xl glow-primary">
              <Link to="/investors">
                {t("cta.investor")}
                <ArrowRight className="w-5 h-5 ml-2" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="h-14 px-8 text-lg rounded-xl border-border hover:bg-accent">
              <Link to="/careers">{t("cta.join_team")}</Link>
            </Button>
            <Button asChild variant="ghost" size="lg" className="h-14 px-8 text-lg rounded-xl">
              <Link to="/contact">{t("cta.contact_us")}</Link>
            </Button>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
            className="mt-16 pt-8 border-t border-border/50">
            <p className="text-sm text-muted-foreground mb-4">{t("cta.trusted_by")}</p>
