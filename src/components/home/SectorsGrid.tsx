    ...
    <section className="py-20 lg:py-32 bg-surface-soft dark:bg-surface-muted">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            {t("sectors.title_1")}
            <span className="gradient-text">{t("sectors.title_2")}</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          {displayedSectors.map((sector, index) => {
            ...
            return (
              <motion.div
                key={sector.id}
                variants={{ hidden: { opacity: 0, y: 30, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 120, damping: 20 } } }}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.05 }}
                className={cn("group relative rounded-2xl overflow-hidden transition-all duration-300", isExpanded ? "sm:col-span-2 lg:col-span-2" : "")}
              >
                <div className="glass-card h-full p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className={cn("w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center text-white", sector.gradient)}>
                  ...
                  <h3 className="text-lg font-semibold text-foreground mb-2 group-hover:text-primary transition-colors">{sector.name}</h3>
                  <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{sector.description}</p>

                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                        <div className="pt-4 border-t border-border">
                          <p className="text-sm text-muted-foreground mb-6">{sector.fullDescription}</p>
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-4">
                    <Button asChild variant="outline" size="sm" className="flex-1 h-9 text-xs">
                      <Link to={`/industries/${sector.slug}`}>
                        {t("sectors.learn_more")}
                        <ArrowRight className="w-3 h-3 ml-1" />
                      </Link>
                    </Button>
                    <Button asChild size="sm" className="flex-1 h-9 text-xs bg-primary hover:bg-primary-dark text-primary-foreground">
                      <a href={`https://${sector.subdomain}`} target="_blank" rel="noopener noreferrer">
                        {t("sectors.visit_site")}
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mt-12">
          {!showAll ? (
            <Button onClick={() => setShowAll(true)} variant="outline" size="lg" className="rounded-xl">
              {t("sectors.view_all").replace("{count}", String(sectors.length))}
              <ChevronDown className="w-4 h-4 ml-2" />
            </Button>
          ) : (
            <Button asChild size="lg" className="rounded-xl bg-primary hover:bg-primary-dark text-primary-foreground">
              <Link to="/industries">
                {t("sectors.explore_all")}
