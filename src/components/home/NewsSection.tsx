    ...
    <section className="py-20 lg:py-32 bg-surface-soft dark:bg-surface-muted">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          {featuredNews && (
            <motion.article initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="lg:row-span-2">
              <Link to={`/news/${featuredNews.slug}`} className="group block h-full glass-card rounded-2xl overflow-hidden">
                <div className="relative aspect-[16/10] lg:aspect-auto lg:h-64 overflow-hidden">
                  ...
                  <span className="absolute top-4 left-4 px-3 py-1 rounded-full bg-primary text-primary-foreground text-xs font-medium">{featuredNews.category}</span>
                </div>
                <div className="p-6 lg:p-8">
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                    <span className="flex items-center gap-1"><Calendar className="w-4 h-4" />{featuredNews.date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-4 h-4" />{featuredNews.readTime}</span>
                  </div>
                  <h3 className="text-xl lg:text-2xl font-bold text-foreground mb-3 group-hover:text-primary transition-colors">{featuredNews.title}</h3>
                  <p className="text-muted-foreground line-clamp-3">{featuredNews.excerpt}</p>
                  <div className="mt-6 flex items-center gap-2 text-primary font-medium">
                    <span>{t("news_section.read_article")}</span>
          ...
          <div className="space-y-4 lg:space-y-6">
            {regularNews.map((item, index) => (
              <motion.article key={item.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                <Link to={`/news/${item.slug}`} className="group flex gap-4 lg:gap-6 glass-card p-4 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="font-semibold text-foreground mb-2 line-clamp-2 group-hover:text-primary transition-colors">{item.title}</h4>
                    <p className="text-sm text-muted-foreground line-clamp-2 hidden sm:block">{item.excerpt}</p>
                  </div>
                </Link>
              </motion.article>
            ))}
