...
export function AIWorkspaceDialog({ open, section, onClose }: { open: boolean; section: WorkspaceKey; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-stretch justify-center sm:items-center sm:p-3" role="dialog" aria-modal="true" aria-label="AI Workspace">
    ...
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-background sm:h-[min(760px,92vh)] sm:max-w-5xl sm:flex-row sm:rounded-2xl sm:border sm:border-border/60 sm:shadow-2xl">
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain">{active === 'projects' ? <AIProjectsPanel /> : active === 'artifacts' ? <AIArtifactsPanel /> : active === 'connectors' ? <AIConnectorsPanel /> : <div className="flex min-h-full items-center justify-center p-6 text-center"><div className="max-w-sm"><p className="text-sm font-medium">Plugins / Skills</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Skill katalogi va enable/disable lifecycle uchun backend registry contract kerak. UI shell keyingi bosqichda ulanadi.</p></div></div>}</main>
    </div>
  </div>;
}
