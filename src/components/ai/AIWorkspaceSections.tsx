...
export function AIWorkspaceSections() {
  ...
  return <div className="space-y-0.5">{sections.map(({ key, label, icon: Icon, empty }) => <WorkspaceSection key={key} storageKey={`alsamos.ai.section.${key}`} icon={<Icon className="h-3.5 w-3.5 text-muted-foreground" />} label={label} empty={empty} />)}</div>;
}

function WorkspaceSection({ storageKey, icon, label, empty }: { storageKey: string; icon: React.ReactNode; label: string; empty: string }) {
