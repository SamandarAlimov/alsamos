...
export function AIWorkspaceSections() {
  return (
    <div className="space-y-1.5" aria-label="AI Workspace bo'limlari">
      {sections.map(({ key, label, icon: Icon, description }) => (
        <WorkspaceSection key={key} workspaceKey={key} icon={<Icon className="h-4 w-4" />} label={label} description={description} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border/40 bg-background/40">
      <div className="flex items-center gap-2 px-2 py-1.5">
        <button type="button" onClick={toggle} className="flex min-w-0 flex-1 items-center gap-2 rounded-lg px-1 py-1 text-left hover:bg-muted/40" aria-expanded={open}>
        ...
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={openWorkspace} aria-label={`${label}ni ochish`}>
          <ArrowUpRight className="h-3.5 w-3.5" />
        </Button>
      </div>
      {open && (
        <button type="button" onClick={openWorkspace} className="mx-2 mb-2 flex w-[calc(100%-1rem)] items-center justify-between rounded-lg bg-muted/40 px-2.5 py-2 text-left hover:bg-muted/70">
          <span className="text-[10px] text-muted-foreground">{label} workspace'ini boshqarish</span>
